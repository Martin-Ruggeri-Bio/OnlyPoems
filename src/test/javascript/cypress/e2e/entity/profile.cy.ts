...
describe('Profile e2e test', () => {
  ...
  const profileSample = { name: 'Gris Toallas', email: 'Sonia37@hotmail.com', handle: 'out-of-the-box' };
  ...
  describe('new Profile page', () => {
    ...
    it('should create an instance of Profile', () => {
      cy.get(`[data-cy="name"]`).type('Administrador visualize').should('have.value', 'Administrador visualize');

      cy.get(`[data-cy="email"]`).type('Enrique.Cordero27@hotmail.com').should('have.value', 'Enrique.Cordero27@hotmail.com');

      cy.get(`[data-cy="handle"]`).type('connecting').should('have.value', 'connecting');
